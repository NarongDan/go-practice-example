...
// import everything which can be changed all the time
type (
	Database struct {
...
func ConfigGetting() *Config {
	once.Do(func() {
		...
		viper.AddConfigPath("./config") // เพิ่ม path
		viper.AutomaticEnv()
		...
		// ✨ Override clientId / clientSecret จาก ENV ถ้ามี
		if cid := os.Getenv("GOOGLE_OAUTH_CLIENT_ID"); cid != "" {
			configInstance.OAuth2.ClientId = cid
		}
		if cs := os.Getenv("GOOGLE_OAUTH_CLIENT_SECRET"); cs != "" {
			configInstance.OAuth2.ClientSecret = cs
		}
